 ...
 * @since 0.4
 */
public class MyPageView extends AbstractUrlBasedView {

    private static final Log LOG = LogFactory.getLog(MyPageView.class);
    ...
    protected MyGSPResponseWriter createResponseWriter(HttpServletResponse response) {
        MyGSPResponseWriter out = MyGSPResponseWriter.getInstance(response);
    ...
    protected void initTemplate() throws IOException {
        ...
        if (template instanceof MyGroovyPageTemplate) {
            ((MyGroovyPageTemplate)template).setAllowSettingContentType(true);
        }
    }
}
