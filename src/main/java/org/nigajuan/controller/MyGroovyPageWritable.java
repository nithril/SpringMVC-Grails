 ...
 * @since 0.5
 */
class MyGroovyPageWritable implements Writable {
    private static final Log LOG = LogFactory.getLog(MyGroovyPageWritable.class);
    ...
    public MyGroovyPageWritable(GroovyPageMetaInfo metaInfo, boolean allowSettingContentType) {
        this.metaInfo = metaInfo;
        this.allowSettingContentType = allowSettingContentType;

        DispatcherServletWebRequest dispatcherServletWebRequest = (DispatcherServletWebRequest) RequestContextHolder.getRequestAttributes();

        //webRequest = new GrailsWebRequest(dispatcherServletWebRequest.getRequest() , dispatcherServletWebRequest.getResponse() , );
